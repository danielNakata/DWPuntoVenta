 ...
 */
package dwpuntoventa.gui.MProveedores;
 ...
 * @author danie_000
 */
public class VCatProveedores extends javax.swing.JInternalFrame {
    ...
    private String[] cols = {
        "claveProveedor-ID Prov-60-i"
            ,"nombre-Proveedor-150-s"
            ,"razonSocial-Razon Social-150-s"
            ,"email-Correo Electronico-150-s"
            ,"telefonos-Telefonos-100-s"
            ,"sactivo-Estatus-100-s"
    };
    
    /**
     * Creates new form VCatArticulos
     */
    public VCatProveedores() {
        initComponents();
        poneColumnas();
        cargaProveedores(Constantes.bsqProvTodos, "%");
        creaListenerFiltro();
    }
    
    public VCatProveedores(int idtienda) {
     ...
     */
    private void cargaProveedores(int tipoBusqueda, String filtro){
        try{
            ...
            RespGralDTO resp = new DAOProveedores().consultaDatosProveedores(param);
            if(resp.getRes() == 1){
                this.llenaTablaProv(resp.getListaProveedorDTO());
            }
        }catch(Exception ex){
            System.out.println(Config.nombreApp+"-"
                    +new java.util.Date().toString()
                    +" Clase: "+this.getClass().toString()
                    +" Metodo: cargaProveedores Ex: "+ ex);
            AppLogger.Logger(Config.nombreApp, 1
                    , this.getClass().toString()
                    , new StringBuffer("Metodo: cargaProveedores Ex:" + ex.toString()));
        }
    }
     ...
     * @param lista 
     */
    private void llenaTablaProv(java.util.ArrayList<ProveedorDTO> lista){
    ...
    private void initComponents() {
        ...
        cmbTipoBsq.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "1-Cve Prov", "2-Nombre", "3-Razon", "4-Todos" }));
